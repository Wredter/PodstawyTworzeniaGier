...
public class horde : NetworkBehaviour {
    ...
    void Start () {
        Debug.Log("is local Plaeyr horde: " + isLocalPlayer);
        ...
        for (int i = 0; i < minionsNumber; i++)
        {
            ...
            obj = Instantiate(hordeMinion, new Vector3(spawnRadius*Mathf.Cos(radius) + Random.value, spawnRadius * Mathf.Sin(radius) + Random.value, 0), Quaternion.identity, gameObject.transform);
            minions.Add(obj);
